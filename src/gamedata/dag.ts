...
export class dag{
	...
	constructor(vertices : string[], edges : [string, string][]){
		...
		if(Object.keys(this.verticesTable).length ==0){
			throw "no vertices in table";
		}
	}
	//string
	add_vertex(name : string){ // cannot remove vertices
	...
	get_vertex_by_name(name : string){
